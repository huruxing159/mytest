...
public class VendorOrderManagementJmesaTemplate extends TableFacadeTemplate {
	...
	protected String[] getColumnProperties() {
		return new String[] { "order.purchaseAttemp.gh_transaction_id", "createTime", "order.purchaseAttemp.server", "order.purchaseAttemp.faction", "order.purchaseAttemp.character", "order.purchaseAttemp.trade_method", "purchaseAttemp.auction_item_name", "event", "vendorUnitPrice",
				"order.purchaseAttemp.gold_amount", "vendorTotalPrice" };
	}
	...
	protected void modifyTable(Table table) {
		...
		HtmlColumn acutionItemName = htmlRow.getColumn("purchaseAttemp.auction_item_name");
		acutionItemName.setTitle("拍卖行物品名");
		acutionItemName.setSortable(false);
